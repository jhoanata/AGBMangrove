...
\begin{table}[htbp]s
   \centering
  \topcaption {Candidate statistical regression models for AGB. When log is the natural logarithm; AGB is the above ground biomass (Mg/ha); BIO9 is the mean temperature of driest quarter (\textcelsius); BIO11 mean temperature of coldest quarter (\textcelsius); BIO16 is the precipitation of the wettest quarter (mm); EVI is the enhanced vegetation index;  Lat latitude (decimal degrees); n is the number of observations; R\textsubscript{a}\textsuperscript{2} is the adjusted coefficient of determination; MSE is the mean squared error; F is the F-statistic calculated; AICc is the akaike information criterion with a correction for finite sample sizes; AICcWt is the Akaike weight}  % requires the topcapt package
...
Estimation of AGB using predicting models showed a limitation in terms of the range of the values for the variables considere in each one. Estimation for places located out of the range of distribution of the data used for the models development, showed erratic values \Jhoa{tengo que ser mas explícita cuando digo valores erraticos?, lo dejo genérico porque no recuerdo bien si el resultado fueron solo valores extremadamente altos o si también incluyó valores en extremo bajos}, that led to made the decission to do the estimations only for places located in the range of distribution of the source data used for the developing of the models. 

Applying this restriction, range of distribution of AGB density estimated from the selected predictive models, comprises values between XXX and 200 Mg ha$^{-1}$. Map distribution of AGB density (Figure 4), shows the highest AGB density values for mangroves located in Cispatá Bay and Ciénaga Grande de Santa Marta (Caribbean coast), as well as San Juan river delta (Pacific coast). Average value of AGB density using predecting models is 75.62 $\pm$ 35.75 Mg ha$^{-1}$.
